...
export default function MapPage() {
  ...
  return (
    <div className="w-full h-screen overflow-hidden bg-[#f0fdf4] font-sans touch-none select-none text-slate-900 relative">
      <Head>
      ...
      {viewMode === 'map' && (
        <>
                ...
                <div className="pointer-events-auto bg-white/95 backdrop-blur-3xl shadow-2xl rounded-full p-2.5 flex items-center border border-white/50">
                   ...
                   <div className="flex gap-3">
                       {['walking', 'cycling', 'driving'].map((m) => (
                           <button key={m} onClick={() => setTransportMode(m as TransportMode)} className={`w-13 h-13 rounded-full flex items-center justify-center text-2xl transition-all ${transportMode === m ? 'bg-green-600 text-white scale-110 shadow-lg' : 'text-slate-300 opacity-60'}`}>{m === 'walking' ? '🚶' : m === 'cycling' ? '🚴' : '🚗'}</button>
                       ))}
                   </div>
                </div>
            </div>
            ...
            <div className="absolute top-32 right-8 z-[1000] flex flex-col gap-6">
                ...
                <button onClick={() => mapRef.current?.locate({setView: true, maxZoom: 15})} className="w-16 h-16 bg-green-600 border-4 border-green-300 rounded-[2rem] shadow-2xl flex flex-col items-center justify-center text-white active:scale-90">
                ...
                <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-sm px-4 pointer-events-none">
                    {showSegments && (
                        <div className="pointer-events-auto bg-white/95 backdrop-blur-xl rounded-[3rem] p-6 shadow-2xl mb-4 max-h-[40vh] overflow-y-auto no-scrollbar border-2 border-white">
                            <h3 className="font-black text-xs uppercase tracking-widest text-green-600 mb-4">Maršruto Atkarpos</h3>
                                    ...
                                    return (
                                        <button key={i} onClick={() => highlight(segCoords)} className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-2xl active:scale-95 transition-all text-left">
                                            <div className="flex items-center gap-4">
                                                <div className="w-6 h-6 bg-green-600 text-white rounded-full flex items-center justify-center font-black text-[9px]">{String.fromCharCode(65 + i - 1)}</div>
                    ...
                    <div className="pointer-events-auto bg-white/95 backdrop-blur-2xl rounded-[3.5rem] p-4 shadow-2xl flex items-center justify-between border-2 border-white">
                        <div onClick={() => setShowSegments(!showSegments)} className="flex-grow flex flex-col pl-6 cursor-pointer group">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black text-green-500 bg-green-50 px-2 py-0.5 rounded-full uppercase">{(routes[selectedRouteIndex]?.summary.totalDistance / 1000).toFixed(1)} km</span>
                            </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setViewMode('navigation')} className="h-16 px-10 bg-green-600 text-white rounded-full font-black text-xl shadow-xl active:scale-95 transition-all">VYKSTAME</button>
                        </div>
                    </div>
                </div>
            )}
          ...
          <div className="absolute inset-0 z-[5000] bg-[#f8fafc] p-10 overflow-y-auto no-scrollbar animate-slide-up">
              ...
              <div className="space-y-6 pb-24">
                  {savedRoutes.map((route) => (
                      <div key={route.id} className="bg-white p-8 rounded-[3.5rem] shadow-sm border-2 border-transparent hover:border-green-200 transition-all flex justify-between items-center group">
                          <div onClick={() => loadSavedRoute(route)} className="flex flex-col flex-grow cursor-pointer">
                              ...
                              <span className="text-green-500 text-xs font-black uppercase tracking-widest mt-1">{(route.distance/1000).toFixed(2)} km • {route.pace} min/km</span>
                          </div>
