    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#E8F5E9', position: 'relative', fontFamily: 'Arial, sans-serif', overflow: 'hidden' }}>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '30px', padding: '60px 50px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', textAlign: 'center', maxWidth: '500px', zIndex: 10 }}>
                <h1 style={{ fontSize: '52px', color: '#1976D2', margin: '0 0 20px 0', fontWeight: 'bold', letterSpacing: '-1px' }}>❄️ TapuTapu</h1>
                <div style={{ display: 'inline-block', backgroundColor: '#1976D2', color: 'white', padding: '12px 24px', borderRadius: '25px', fontSize: '14px', fontWeight: 'bold', marginBottom: '40px', letterSpacing: '1px' }}>PAVASARIO METAS 🌸</div>
                ...
                <div style={{ marginTop: '40px', fontSize: '12px', color: '#BDBDBD', fontWeight: 'bold', letterSpacing: '0.5px' }}>VERSIJA 12.3 SPRING EDITION</div>
            </div>
            <style>{`@keyframes float { 0%, 100% { transform: translateY(0px) rotate(0deg); } 50% { transform: translateY(-30px) rotate(10deg); } }`}</style>
        </div>
    );
};
